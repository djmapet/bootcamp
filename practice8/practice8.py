...
m_medal_sort = sorted(m_got_medal_most,key=lambda x:x[0],reverse=True)
...
f_medalist_name_list = list(map(lambda x:x[3] and x[0] == 'F' and x[4],f_medal_list))
f_some_got_medal = collections.Counter(f_medalist_name_list)
f_got_medal_most = f_some_got_medal.most_common()
f_medal_sort = sorted(f_got_medal_most,key=lambda x:x[0],reverse=True)
